<template>
    <section class="w3l-ab-features py-5">
        <div class="container py-md-5 py-3">
            <div class="header-top ml-auto pr-lg-5">
                <h6 class="sub-titlehny mb-2">Features</h6>
                <h3 class="hny-title mb-4">An inspired approach to quality residential projects </h3>
                <!-- <p class="pr-lg-5">Lorem ipsum viverra feugiat. Pellen tesque libero ut justo,
                    ultrices in ligula. Semper at tempufddfel. Lorem ipsum dolor sit amet consectetur adipisicing
                    elit. Non quae, fugiat consequatur voluptatem nihil ad. Lorem ipsum dolor sit amet.</p> -->
            </div>
            <div class="row features-w3pvt-main mt-5 pt-md-3" id="features">
                <div class="col-lg-4 col-md-6 feature-gird d-flex">
                    <!-- <div class="row features-hny-inner-gd"> -->
                    <!-- <div class="col-md-2 col-2 featured_grid_left">

                        </div> -->
                    <!-- <div class="col-md-10 col-10 featured_grid_right_info "> -->
                    <div style="display: flex; margin-right: 30px" class="icon_left_grid">
                        <!-- <span class="fa fa-linode" aria-hidden="true"></span> -->
                        <img style="height: 70px; width: 70px;" src="../../assets/icons/new-product.png" alt="">
                    </div>
                    <h4 class=""><a class="link-hny tags" href="#url">Powerfull Product
                            Strategy </a></h4>
                    <!-- <p>Lorem ipsum dolor sit amet elit consec tetur adipisi elit., rem!</p> -->
                    <!-- </div> -->
                    <!-- </div> -->
                </div>
                <div class="col-lg-4 col-md-6 feature-gird mt-md-0 mt-4 d-flex">
                    <!-- <div class="row features-hny-inner-gd">
                        <div class="col-md-2 col-2 featured_grid_left">
                        </div>
                        <div class="col-md-10 col-10 featured_grid_right_info"> -->
                    <div style="display: flex; margin-right: 30PX" class="icon_left_grid">
                        <!-- <span class="fa fa-pied-piper-alt" aria-hidden="true"></span> -->
                        <img style="height: 70px; width: 70px;" src="../../assets/icons/experience.png" alt="">
                    </div>
                    <h4 class=""><a class="link-hny tags" href="#url">Expertise, discipline & precision </a>
                    </h4>
                    <!-- <p>Lorem ipsum dolor sit amet elit consec tetur adipisi elit., rem!</p> -->
                    <!-- </div>
                    </div> -->
                </div>
                <div class="col-lg-4 col-md-6 feature-gird mt-lg-0 mt-md-5 mt-4 d-flex">
                    <!-- <div class="row features-hny-inner-gd">
                        <div class="col-md-2 col-2 featured_grid_left">
                        </div>
                        <div class="col-md-10 col-10 featured_grid_right_info"> -->
                    <div style="display: flex; margin-right: 30PX" class="icon_left_grid">
                        <!-- <span class="fa fa-pied-piper-alt" aria-hidden="true"></span> -->
                        <img style="height: 70px; width: 70px;" src="../../assets/icons/satisfaction.png" alt="">
                    </div>
                    <h4 class=""><a class="link-hny tags" href="#url">100% Satisfaction
                            Guarantee</a></h4>
                    <!-- <p>Lorem ipsum dolor sit amet elit consec tetur adipisi elit., rem!</p> -->
                    <!-- </div>
                    </div> -->
                </div>
            </div>
        </div>
    </section>
</template>